...
const WeekMenuModal = ({
  ...
  const handleSave = async () => {
    // Parse weekData.range like "Jul 28 - Aug 1"
    const [rawStart] = weekData.range.split(" - ");
    const currentYear = new Date().getFullYear();
    const parsedMonday = parse(
      `${rawStart} ${currentYear}`,
      "MMM d yyyy",
      new Date(),
    );

    const monday = format(parsedMonday, "yyyy-MM-dd");
    const friday = format(addDays(parsedMonday, 4), "yyyy-MM-dd");

    const payload = {
      startDate: monday,
      endDate: friday,
      monday: assignedMenus.monday?.id || null,
      tuesday: assignedMenus.tuesday?.id || null,
      wednesday: assignedMenus.wednesday?.id || null,
      thursday: assignedMenus.thursday?.id || null,
      friday: assignedMenus.friday?.id || null,
    };

    try {
      console.log("➡️ Sending payload:", payload);
      const response = await axios.post(`${BASE_URL}/menuSchedule`, payload);
      console.log("✅ Week menu saved:", response.data);
